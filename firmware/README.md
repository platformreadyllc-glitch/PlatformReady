...
## Networking: Ethernet-primary

The W5500 module ([network.cpp](src/network.cpp)) is tried first at boot (`networkTryEthernet()`); WiFi is only ever associated when no Ethernet link is present, either at boot (`networkBeginWiFi()`/`runWiFiManager()`) or as a runtime fallback if a previously-good Ethernet link is lost mid-operation (`transportDecide()`, [transport_fsm.h](src/transport_fsm.h)/`.cpp` - pure decision logic, covered by `pio test -e native`; wired into `main.cpp`'s `loop()`). Ethernet, once linked, always takes precedence — a fallback-to-WiFi unit reclaims Ethernet automatically as soon as the link comes back, rather than staying on WiFi. Both transitions are debounced (~1s of stable link state) so a marginal/bouncing cable doesn't thrash between the two, and re-probing Ethernet while on the WiFi fallback is rate-limited (~5s) since that means a real `Ethernet.begin()`/DHCP attempt, not just a link-status read.

Every networked path — REST (`api.cpp`), OTA (`ota.cpp`), and the WebSocket connection below — follows whichever transport is currently active, via `networkNewClient()`/`networkIsEthernet()`.

## Live connection (WebSocket)

Once registered, the device opens a persistent WebSocket connection to the backend (`/esp32-ws?remoteId=...&transport=wifi|ethernet`, see [ws_client.cpp](src/ws_client.cpp) and the backend's `esp-remotes.gateway.ts`), used for three things: the backend's live "connected" status for this remote (a 2s server-side heartbeat, much faster than any HTTP-polling TTL could reasonably be), immediate assignment updates when the remote is reassigned via the management page (no reboot needed), and a live feed of the other active referees' votes on the same platform (for the on-screen mini-scoreboard).

**Transport-agnostic**: the `WebSocketsClient` library (`links2004/WebSockets`) normally manages its own internal `WiFiClient` with no way to inject a generic `Client*` — worked around via its `WEBSOCKETS_NETWORK_TYPE=NETWORK_CUSTOM` extension point ([ws_network_client.cpp](src/ws_network_client.cpp)), which dispatches each new connection to whichever transport is active at connect time, same as REST/OTA. A runtime transport switch (`main.cpp`'s `loop()`) forces a prompt reconnect on the new transport via `wsNotifyTransportChanged()`, rebuilding the URL so the reported `transport` doesn't go stale. `ws_network_client_secure.cpp` is a deliberate dead-but-required stub — `NETWORK_CUSTOM` unconditionally requires a secure-client class to exist and link, even though this project never uses TLS (`ws://` only, private LAN, no `beginSSL()`/`wss://` calls anywhere).

**Dependency note**: `platformio.ini` pins `WebSockets` to a specific GitHub commit rather than the registry package — the published registry snapshot for the version we need is missing two methods (`setCertificate()`/`setPrivateKey()`) that `WebSocketsClient.cpp` requires to exist (though unreachable here) to compile at all under `NETWORK_CUSTOM` on ESP32.
