...
**WiFi-only — known gap on Ethernet**: unlike the OTA and REST API paths (both routed through `networkNewClient()`, working over either WiFi or Ethernet), the `WebSocketsClient` library manages its own internal `WiFiClient` and doesn't accept an injected generic `Client*` — so this connection specifically only works over WiFi. Ethernet is enabled (as of firmware 1.5.8), and `setup()` only associates WiFi when Ethernet *isn't* up — so a unit that connects via Ethernet never opens this WS connection at all: voting/registration/OTA keep working fine over Ethernet, but liveness, live reassignment-without-reboot, and the mini-scoreboard silently don't work on that unit. Needs a real fix, not yet planned in detail — see the pending Claude memory note on this.
